...
export const getStaticProps: GetStaticProps<LocationDetailProps> = async ({
  params,
}) => {
  ...
  try {
    ...
    // Fetch the specific location data using the ID
    const location = await fetchLocationById(id);
    const data = await fetchLocations(1);
    const locationsResults: LocationType[] = data.locations.results;

    return {
      props: { location: location, locations: locationsResults },
    };
  } catch (error) {
